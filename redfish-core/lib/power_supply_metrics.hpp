...
inline void getValues2(const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                       ...
                       averageMaxArray& retValues)
{
    const std::array<std::string, 1> interfaces = {interfaceName};
    crow::connections::systemBus->async_method_call(
        [aResp, chassisID, powerSupplyID, interfaceName, retValues](
            ...
            for (const auto& [objectPath, serviceName] : intfSubTree)
            {
                ...
                if (psuInputPowerStr == psuMatchStr)
                {
                    ...
                    crow::connections::systemBus->async_method_call(
                        [aResp, chassisID, powerSupplyID, retValues](
                            ...
                            // retValues =
                            // std::get<averageMaxArray>(*intfValues);
                            copy(intfValuesPtr->begin(), intfValuesPtr->end(),
                                 back_inserter(retValues));
                            ...
                            for (const auto& values : *intfValuesPtr)
                            {
                                ...
                                BMCWEB_LOG_DEBUG << "Values: "
                                                 << std::get<1>(values);
                                // retValues.push_back(values);
                                // retValues.push_back(averageMaxEntry(
                                //    std::get<0>(values),
                                //    std::get<1>(values)));
                                // retValues.emplace_back(averageMaxEntry(
                                //    std::get<0>(values),
                                //    std::get<1>(values)));
                            }
                            // retValues = *intfValuesPtr;
                            // retValues =
                            //    const_cast<averageMaxArray&>(*intfValuesPtr);
                            // retValues =
                            //    std::get_if<averageMaxArray>(intfValues);
                            // retValues =
                            // std::get<averageMaxArray>(intfValues);
